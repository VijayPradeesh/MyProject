...
import { REPORTS_HEADER } from '../../../constants/tableHeaders';
        ...
        this.props.getReports(bodyParameters).then((response) => {
            ...
            if (response.status == 200) {
                this.setState({
                    ...
                    reports: response.data.reportData
                }, () => {
                ...
                <Grid container sx={{ backgroundColor: '#f6f6f6' }}>
                    ...
                    <Grid item xs={12} className={classes.actionTab}>
                        <Grid container className={classes.actionTabGrid}>
                            <Grid item xs={12} height={'60px'} sx={{ alignItems: 'center', display: "flex" }}>
                                <Typography style={{ fontSize: '22px', fontWeight: 600, marginLeft: '38px' }} className={classes.userAccessText}> Daily Activity Report </Typography>
                            </Grid>
                            {/* Filters */}
                    ...
                    <Grid sx={{ display: { xs: 'none', sm: 'flex', minHeight: 'calc(100vh - 190px )' } }} item xs={12} padding="20px" className={classes.flexStartGrid}>
                        <ReactVirtualizedTable
                            ...
                            {isLoading ? <SkeletonComponent columns={REPORTS_HEADER} /> :
                                reports && dropDown &&
                                stableSort(reports, getComparator(order, orderBy))
                                    .map((list, i) => (
                                        <TableRow style={{ height: '90px' }}>
                                            <TableCell style={{ paddingTop: 5, paddingBottom: 5, minWidth: '100px' }}>{list.jobNumber} </TableCell>
                                            ...
                                            <TableCell style={{ paddingTop: 5, paddingBottom: 5, minWidth: '70px' }}>{list.payItemName} </TableCell>
                                            <TableCell style={{ paddingTop: 5, paddingBottom: 5 }}>{list.payItemDecsription} </TableCell>
                                            <TableCell style={{ paddingTop: 5, paddingBottom: 5, minWidth: '50px' }}>{list.purchaseOrder} </TableCell>
                                            <TableCell style={{ paddingTop: 5, paddingBottom: 5 }}>{list.quantity} </TableCell>
                                            ...
                                            <TableCell style={{ paddingTop: 5, paddingBottom: 5 }}>{list.uom} </TableCell>
                                            <TableCell style={{ paddingTop: 5, paddingBottom: 5, minWidth: '200px' }}>{list.wbsDescription} </TableCell>
                                            <TableCell style={{ paddingTop: 5, paddingBottom: 5, minWidth: '200px' }}>
                                                <CustomInput
                                                    // inputProps={{ maxLength: 100 }}
                                                    name="comments"
                                                    //label={`Comments ${approvalVal === "Reject" ? "(required)" : ""}`}
                                                    fieldType="textarea"
                                                    readOnly
                                                    hideOutline
                                                    maxRows={3}
                                                    value={list.foremanComments}
